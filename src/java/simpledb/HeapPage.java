...
public class HeapPage implements Page {
    ...
    */
    private int getNumTuples() {        
        ...
        final int tupleSize = td.getSize();
    ...
    public void deleteTuple(Tuple t) throws DbException {
        ...
        final int tupleno = t.getRecordId().getTupleNumber();
    ...
    /**
     * Count the set bit in a byte
     * @param theByte the byte
     * @return the number of 1 in that byte
     */
    private static int countBits(byte theByte) {
        int count = 0;
        while (theByte != 0) {
            if ( (theByte & mask[7]) != 0 ) {
                count++;
    ...
    public int getNumEmptySlots() {
        ...
        for (byte slotByte : header) {
            usedSlotNumber += countBits(slotByte);
        }
        return this.numSlots - usedSlotNumber;
    }
    
    private static final byte[] mask = {1, 2, 4, 8, 16, 32, 64, -128};

    /**
     * Returns true if associated slot on this page is filled.
     */
    public boolean isSlotUsed(int i) {
        assert i < this.numSlots : "Index exceeds page capacity";
        final byte slotByte = header[i/8];
        return ( (slotByte & mask[i%8]) != 0 );
    }
