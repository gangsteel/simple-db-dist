...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if (joinOp == Predicate.Op.EQUALS || joinOp == Predicate.Op.LIKE) {
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        for (int i = 1; i <= joins.size(); i++) {
            ...
            for (Set<LogicalJoinNode> subset : allSubsets) {
                CostCard bestCC = new CostCard();
                bestCC.cost = Double.POSITIVE_INFINITY;
                for (LogicalJoinNode node : subset) {
                    CostCard cc = computeCostAndCardOfSubplan(stats, filterSelectivities, node, subset, bestCC.cost, pc);
                    if (cc == null) continue;
                    if (cc.cost < bestCC.cost) {
                        bestCC = cc;
                    }
                }
        ...
        joins = pc.getOrder(new HashSet<LogicalJoinNode>(joins));
        return joins;
    }
